...
class HCDVectorStoreComponent(LCVectorStoreComponent):
    ...
    def build_vector_store(self):
        ...
        try:
            ...
            raise ValueError(f"Invalid setup mode: {self.setup_mode}") from e
        ...
        if not isinstance(self.embedding, dict):
            ...
            dict_options = {
                k: v for k, v in self.embedding.get("collection_vector_service_options", {}).items() if k and v
            }
        ...
        metadata_indexing_include = self.metadata_indexing_include
        metadata_indexing_exclude = self.metadata_indexing_exclude
        collection_indexing_policy = self.collection_indexing_policy

        if metadata_indexing_include:
            vector_store_kwargs["metadata_indexing_include"] = metadata_indexing_include
        elif metadata_indexing_exclude:
            vector_store_kwargs["metadata_indexing_exclude"] = metadata_indexing_exclude
        elif collection_indexing_policy:
            vector_store_kwargs["collection_indexing_policy"] = collection_indexing_policy
    ...
    def _add_documents_to_vector_store(self, vector_store) -> None:
        documents = [_input.to_lc_document() for _input in self.ingest_data or [] if isinstance(_input, Data)]
