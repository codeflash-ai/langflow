...
class AddContentToPage(LCToolComponent):
    ...
    def process_node(self, node):
        ...
        extend_blocks = blocks.extend

        def extract_header(heading_level, heading_text):
            if heading_level in range(3):
                append_block(self.create_block(f"heading_{heading_level + 1}", heading_text))
        ...
        if isinstance(node, str):
            ...
            if text:
                if text.startswith("#"):
                    ...
                    extract_header(heading_level, heading_text)
                else:
                    append_block(self.create_block("paragraph", text))
        elif node.name in {"h1", "h2", "h3"}:
            append_block(self.create_block(f"heading_{int(node.name[1])}", node.get_text(strip=True)))
        elif node.name == "p":
            code_node = node.find("code")
            if code_node:
                code_text = code_node.get_text()
                language, code = self.extract_language_and_code(code_text)
                append_block(self.create_block("code", code, language=language))
            elif self.is_table(str(node)):
                extend_blocks(self.process_table(node))
            else:
                append_block(self.create_block("paragraph", node.get_text(strip=True)))
        elif node.name in {"ul", "ol"}:
            list_type = "bulleted_list_item" if node.name == "ul" else "numbered_list_item"
            extend_blocks(self.process_list(node, list_type))
        elif node.name == "blockquote":
            append_block(self.create_block("quote", node.get_text(strip=True)))
        elif node.name == "hr":
            append_block(self.create_block("divider", ""))
        elif node.name == "img":
            append_block(self.create_block("image", "", image_url=node.get("src")))
        elif node.name == "a":
            append_block(self.create_block("bookmark", node.get_text(strip=True), link_url=node.get("href")))
        ...
        for child in node.children:
            if isinstance(child, str):
                continue
            extend_blocks(self.process_node(child))
    ...
    def create_block(self, block_type: str, content: str, **kwargs) -> dict[str, Any]:
        ...
        if block_type in rich_text_block_types:
            ...
            block[block_type] = {
                "table_width": kwargs.get("table_width", 0),
