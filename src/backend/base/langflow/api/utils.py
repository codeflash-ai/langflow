...
def build_input_keys_response(langchain_object, artifacts):
    ...
    # If the object has memory, remove memory variables from input keys and add to memory_keys
    if hasattr(langchain_object, "memory") and hasattr(langchain_object.memory, "memory_variables"):
        memory_vars = set(langchain_object.memory.memory_variables)
        memory_keys = list(memory_vars)
        input_keys = {key: value for key, value in input_keys.items() if key not in memory_vars}
